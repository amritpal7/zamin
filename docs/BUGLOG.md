...
| **State in a `useFocusEffect` callback re-fires the effect on every change** | 1 | `useFocusEffect(useCallback(fn, [load]))` re-runs `fn` whenever `load` changes *while focused*. If `load` depends on a fast-changing value (e.g. `search` bound to a TextInput), it fires per keystroke (request + list reset). Pin such values in a ref and drop them from the callback's deps; trigger the action explicitly (onSubmit) or keep only the deps that *should* reload (filter chips). |
...
| **`expo-blur` doesn't blur on Android → translucent "glass" looks washed-out** | 1 | On Android `BlurView` renders a **flat translucent tint** (no real blur) unless `experimentalBlurMethod="dimezisBlurView"` is set — so translucent surface tokens (`glassBg`, `glass.fill`) + white sheens come out muddy/unprofessional (iOS blurs natively, so it hides there). **Design rule:** iOS/web = Liquid Glass; **Android = opaque Material surfaces** (theme overrides the glass tokens to opaque on Android) + at most one genuinely-blurred element (nav bar) via `experimentalBlurMethod`. Gate white specular sheens with `Platform.OS !== "android"`. All glass goes through `LiquidGlass` (which branches per platform). |
| **Postgres `DECIMAL/NUMERIC` → JS strings (pg default) → native crash** | 1 | node-postgres serializes `DECIMAL/NUMERIC` (OID 1700) as **strings** (avoids float precision loss). Our `latitude/longitude` are `DECIMAL`, so the API emitted `"18.559"`; react-native-maps (`region`/`Marker`/`Polygon`) then throws `UnexpectedNativeTypeException: … cannot be cast from String to double` on **Android** (web maps are stubbed → never surfaced there; test native + consumer-side). Fixed at source with `pg.types.setTypeParser(1700, parseFloat)` in `backend/src/db.js` (safe — lat/lng are our only NUMERIC cols) + client `num()` coercion before any map coord. Guardrail: values from the DB via `pg` aren't guaranteed to be JS numbers — coerce anything feeding a native view. |
